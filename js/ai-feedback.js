...
async function showApiKeyModal(onSave) {
  ...
  saveBtn.onclick = async () => {
    ...
    try {
      ...
      const res = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-flash-latest?key=${key}`);
...
3. ถ้าไม่มี Expected Output: ตรวจว่าโค้ดนักเรียนทำได้ตรงตามคำอธิบายโจทย์หรือไม่
4. ถ้าโค้ดนักเรียนใช้ **วิธีอื่น** ที่ต่างจากเฉลยแต่ได้ผลลัพธ์ถูกต้องเหมือนกัน → ถือว่า **ถูกต้อง** ให้คะแนนเต็มได้
